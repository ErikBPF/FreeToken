...
def test_copy_missing_can_select_one_bank(monkeypatch):
    calls = []
    gate_source, gate_cache = torch.empty(1), torch.empty(1)
    down_source, down_cache = torch.empty(1), torch.empty(1)
    cache = SimpleNamespace(
        banks=[([gate_source], gate_cache), ([down_source], down_cache)],
        _pending_src_layer=0,
        _pending_whole_layer=False,
        _unpinned_layers=frozenset(),
        _copy_fused_ok=True,
        evict_slots=torch.empty(1),
        src_indices=torch.empty(1),
        num_indices=torch.empty(1),
    )
    monkeypatch.setattr(
        "freetoken.kernel.fast_index_copy_jit",
        lambda dst, *_args: calls.append(dst),
    )

    OffloadMoeCache.copy_missing(cache, bank_indices=(1,))

    assert calls == [down_cache]
